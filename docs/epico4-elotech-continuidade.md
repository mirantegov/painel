...
## Tabelas no manifest (59) — por domínio (schema siscop)
...
- **Despesa execução:** empenho, anulacaoempenho, cancelamentorestos, fichaempenho, liquidacao, estornoliquidacao, pagamento, estornopagamento, ordempagamento, ordempagamento_liquidacao, retencoesliquidacao, lancamentosequencia.
- **Extra-orçamentário:** cadastroextra, extraorcamentario, estornoextraorcamentario, pagamentoextra, estornopagamentoextra.
...
Não há matview em siscop. Das 82 views, as relevantes (despesa/receita/financeiro) referenciam
tabelas-base — já cobertas, + estas **adicionadas** ao manifest: `ordempagamento` (cabeçalho da OP,
de `listapagamento`) e o bloco **extra-orçamentário** (`cadastroextra`, `extraorcamentario`,
`estornoextraorcamentario`, `pagamentoextra`, `estornopagamentoextra` — de `listapagamento`).
Agregados `despesamensal`/`despesatrimestral` (usados em `anexo09`/`orcanexo09`) **não** foram
incluídos: são deriváveis das movimentações no ClickHouse (incluir só se o painel exigir o agregado pronto).

## Em aberto — PERGUNTAR ao usuário

(Valores do teste já definidos — ver acima: 4117107/1/2026.)

1. **Conciliação detalhada:** `conciliacaoextrato`/`conciliacaoextratoitem` **não têm coluna `entidade`** (ligam por id de conciliação). Incluir como global (dump de todos clientes, pesado) ou filtrar por outra chave? Precisa de query de referência.
...
1. **Rodar o 1º export** (alvo já definido — comando com `--var` na seção "Alvo do teste"). Usuário executa na máquina que alcança o banco Elotech; conferir objetos em `mc ls -r local/mirante-parquet/4117107/` e ler 1 Parquet de volta (`go run ./cmd/inspect`).
2. **Resolver os itens "Em aberto"** com o usuário; ajustar manifest.
3. **Iniciar Épico 5 (ClickHouse):** portar os marts `ctb_*`/`orc_*` do projeto do cliente (queries "Despesa Executada/consolidada" já mapeadas em `docs/epico5-elotech-queries.md`) — raw siscop (MinIO) → SIM-AM. O cliente já tem a lógica pronta em ClickHouse, acelera muito.
