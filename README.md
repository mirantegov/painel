...
- [x] **E6** MinIO + exportador Go — MinIO via Traefik (`minio.mirantegov.cloud`); exportador pronto: **3 manifests** (siscop/aise/apice) com filtros DDL-derivados, `exclude_columns` (bytea), **gerador versionado** ([`gen_manifests.py`](exporter/tools/gen_manifests.py)) e pacote `make package`. siscop já exportado+importado p/ Nova Londrina; falta rodar aise/apice no cliente. Docs: [runbook](docs/runbook-exportador.md) · [gerador](docs/gerador-manifests.md) — [MIR-9](https://linear.app/code42dev/issue/MIR-9) · [#126](https://github.com/mirantegov/painel/issues/126)
  - [ ] **Backlog (hoje, 2026-06-10): migrar storage MinIO → AWS S3 nativo** — menos latência e mais banda na importação. Trocar `S3_ENDPOINT`/região/credenciais no exportador Go e no `infra/clickhouse/tools/import_raw.sh` (função `s3(...)` apontando p/ bucket S3), atualizar `docs/runbook-exportador.md`, `.env.production` e `docs/ACESSOS.local.md`. Validar import `s3()` no ClickHouse contra o bucket S3 e medir ganho vs MinIO.
- [ ] **E7** ClickHouse SIM-AM, ETL e marts — `raw_<ibge>`/`sim_<ibge>` provisionados, RAW siscop importado; **falta** ETL `raw→sim` (reimplementar lógica das views) — [MIR-10](https://linear.app/code42dev/issue/MIR-10) · [#127](https://github.com/mirantegov/painel/issues/127)
