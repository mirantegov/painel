...
# The agent passes tool input via stdin as JSON.

INPUT=$(cat)

if echo "$INPUT" | grep -qE '"(file_path|path)"\s*:\s*"[^"]*components/ui/'; then
  echo "" >&2
  echo "ATENCAO: components/ui/ contem primitivos shadcn/ui." >&2
  echo "   Estes arquivos NAO devem ser modificados sem necessidade explicita e documentada." >&2
